...
- **Navigation mode (track-up)** — the `Navigation2` button (bottom-right control column, above the follow button) toggles a car-navigation-style mode using the `leaflet-rotate` plugin (map init: `rotate: true`, other rotation handlers off; rotation set via `map.setBearing(-heading)`). It centers on the GPS position at `NAV_ZOOM` (17) and rotates the map so the direction of travel points up. Heading comes from the Geolocation `heading`/`speed` on `locationfound` (falls back to the bearing between the last two positions when stationary/unavailable) and is angle-smoothed in `headingRef`. The position marker switches to an up-pointing arrow (`navArrowIcon`) that always indicates forward (leaflet-rotate keeps markers screen-upright). Dragging the map or pressing the plain follow button exits nav mode and resets bearing to 0. Note: Leaflet is 2D — this is rotation only, **not** 3D/tilt perspective (that would need MapLibre GL).
